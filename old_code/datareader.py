...
class Camera_Wrapper_nvc():
    def __init__(self,sequence,ds = 2, device_idx = 0):
        ...
        checksum_path="/home/derek/Documents/derek/test-scripts/ts/timestamp_pixel_checksum_6_h18.pkl"
        geom_path="/home/derek/Documents/derek/test-scripts/ts/ts_geom_h18.pkl"
        
        checksum_path2="/home/derek/Documents/derek/test-scripts/ts/timestamp_pixel_checksum_6_h12.pkl"
        geom_path2="/home/derek/Documents/derek/test-scripts/ts/ts_geom_h12.pkl"
        ...
        self.nvDec = nvc.PyNvDecoder(sequence, gpuID)
        self.target_h, self.target_w = self.nvDec.Height(), self.nvDec.Width()

        self.to_rgb = nvc.PySurfaceConverter(self.nvDec.Width(), self.nvDec.Height(), nvc.PixelFormat.NV12, nvc.PixelFormat.RGB, gpuID)
        self.to_planar = nvc.PySurfaceConverter(self.nvDec.Width(), self.nvDec.Height(), nvc.PixelFormat.RGB, nvc.PixelFormat.RGB_PLANAR, gpuID)
    ...
    def __next__(self):
        ...
        rawSurface = self.nvDec.DecodeSingleSurface(pkt)
        if (rawSurface.Empty()):
            print("Error grabbing packet, are you at the last frame?")
        
        # Convert to RGB interleaved;
        rgb_byte = self.to_rgb.Execute(rawSurface, self.cc_ctx)
    
        # Convert to RGB planar because that's what to_tensor + normalize are doing;
        rgb_planar = self.to_planar.Execute(rgb_byte, self.cc_ctx)
        ...
        surface_tensor.resize_(3, self.target_h,self.target_w)
        
        # try:
        #     surface_tensor = torch.nn.functional.interpolate(surface_tensor.unsqueeze(0),self.resize).squeeze(0)
        # except:
        #     raise Exception("Surface tensor shape:{} --- resize shape: {}".format(surface_tensor.shape,self.resize))
        ...
        surface_tensor = surface_tensor.permute(1,2,0)
        print(surface_tensor.device)
        self.frame = surface_tensor.data.cpu().numpy()
    ...
    def __len__(self):
        print("Warning: queried video sequence length but this function is not implemented in {}".format(self))
        return 10000
